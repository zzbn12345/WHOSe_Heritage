...
class OuvDataset(Dataset):
    ...
    @classmethod
    def load_dataset_and_make_vectorizer(cls, ouv_csv, vocab):
        ...
        return cls(ouv_df, OuvVectorizer.from_dataframe(vocab=vocab), vocab=vocab)
...
def load_encoder(model, file, device=None):
        ...
        distrib_barrier()
        wgts = torch.load('model_storage/rnn/ulmfit/models/' + file+'.pth', map_location=device)
...
def initialization(vocab):
    if args.reload_from_files:
        ...
        dataset = OuvDataset.load_dataset_and_make_vectorizer(args.ouv_csv, vocab=vocab)
...
def training_loop(vocab, verbose=False):
    
    dataset,vectorizer,classifier = initialization(vocab)
    ...
    try:
        for epoch_index in range(args.num_epochs):
            ...
            if epoch_index == 0:
                ...
                classifier = load_encoder(classifier, 'finetuned').to(args.device)
            elif epoch_index == 1:
                args.learning_rate = 1e-2
                freeze_to(classifier,-1)
            elif epoch_index == 2:
                args.learning_rate = 1e-3
                freeze_to(classifier,-2)
            elif epoch_index == 3:
                args.learning_rate = 1e-4
                freeze_to(classifier, -3)
            else:
                args.learning_rate = 2e-5
                freeze_to(classifier, 0)
...
def fuzzy_exp(hyperdict,vocab, verbose):
    '''
    Perform a hyperparameter search using grid search and save into the hyperdict
    '''
    s_fuzzy_how = ['uni']
    ...
    for how in s_fuzzy_how:
        for lbd in s_fuzzy_lambda:
            ...
            if not key in hyperdict:
                train_state = training_loop(vocab=vocab, verbose=verbose)
                hyperdict[key] = train_state
...
def infer_tokens_importance(vocab, classifier, vectorizer, classes, k=50):
    ...
    vectorized_token = []    
    for token in vocab:
        vectorized_token.append(torch.tensor(vectorizer.vectorize(token)[0]))
...
def main():
    ...
    for sd in s_seed:
        ...
        fuzzy_exp(hyperdict = hyperdict_fuzzy, vocab=vocab, verbose=True)

        with open(args.save_dir+'hyperdict_fuzzy.p', 'wb') as fp:
            pickle.dump(hyperdict_fuzzy,fp, protocol=pickle.HIGHEST_PROTOCOL)
    
    #vocab = get_ngram_vocab()
